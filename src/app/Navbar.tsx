...
const iconClass = "block h-5 w-5 shrink-0 fill-current";
  ...
  },
  {
    ...
    icon: (
      <svg
        viewBox="0 0 24 24"
        aria-hidden="true"
        className={`${iconClass} translate-y-1`}>
        ...
        <ul className="absolute left-1/2 flex -translate-x-1/2 items-center gap-6">
          {navLinks.map(({ href, label }) => (
            <li key={label}>
              <Link
                href={href}
                className="text-sm font-medium transition hover:opacity-80 hover:underline underline-offset-4">
                {label}
              </Link>
            </li>
          ))}
          ...
          {social.map(({ href, label, icon }) => (
            <li key={label} className="shrink-0">
              <a
                ...
                className="flex h-9 w-9 shrink-0 items-center justify-center transition hover:scale-110">
                {icon}
              </a>
            </li>
          ))}
